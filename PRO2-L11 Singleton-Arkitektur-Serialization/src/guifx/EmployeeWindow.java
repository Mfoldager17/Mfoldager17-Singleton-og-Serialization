...
public class EmployeeWindow extends Stage {
	...
	private void initContent(GridPane pane) {
		...
		cbbCompany.getItems().addAll(c.getCompanies());
	...
	private void okAction() {
		...
		if (employee != null) {
			c.updateEmployee(employee, name, wage);
			if (companyIsSelected) {
				c.addEmployeeToCompany(employee, newCompany);
			} else {
				c.removeEmployeeFromCompany(employee, employee.getCompany());
			}
		} else {
			if (companyIsSelected) {
				c.createEmployee(name, wage, newCompany);
			} else {
				c.createEmployee(name, wage);
			}
		}
		hide();
	}
